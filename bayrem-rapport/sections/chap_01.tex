...
\begin{itemize}
    \item Implement hybrid authentication: Keycloak for administrator access control and Spring Security with JWT for customer/agency authentication.
...
\begin{itemize}
    ...
    \item Integrate Flask for the AI chatbot service with OpenAI integration.
\end{itemize}
...
\begin{itemize}
  \item \textbf{Hybrid Security Architecture}: Keycloak for admin authentication and role management; Spring Security with JWT for customer and agency authentication.
  \item \textbf{Admin-Agency Chat System}: HTTP polling-based chat (30-second refresh) for real-time communication between administrators and agencies.
  \item \textbf{Automated Notification and Email System}: Sends automatic updates for bookings, new cars, and account activities.
  \item \textbf{Contract Generation with QR Code}: PDF rental agreements generated and sent via email upon booking approval.
  ...
  \item \textbf{Integrated AI Chatbot}: Flask-based chatbot using OpenAI GPT for car recommendations and customer assistance.
\end{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{img/system-architecture-overview.png}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{Chatbot Service (Flask)}: AI-powered assistant integrated with OpenAI GPT API for car recommendations, FAQ responses, and customer support.
    \end{itemize}
    
    \item \textbf{Security Layer}:
    \begin{itemize}
        \item \textbf{Keycloak}: OAuth2/OpenID Connect server for administrator authentication and role management (RBAC).
        \item \textbf{Spring Security + JWT}: Token-based authentication for customers and agencies, with password encryption and CORS configuration.
    \end{itemize}
...
\begin{enumerate}
    ...
    \item For Admin: Backend validates token with Keycloak server.
    \item For Customer/Agency: Spring Security validates JWT token internally.
    \item Business logic executes, database queries processed.
    \item For AI assistance: Frontend calls Flask chatbot API (HTTP POST).
    \item For Admin-Agency chat: HTTP polling every 30 seconds retrieves new messages.
    \item Responses returned to frontend, UI updated.
\end{enumerate}

\subsubsection{Containerization and Deployment}
\begin{itemize}
    \item \textbf{Docker}: Each service (Angular, Spring Boot, Flask Chatbot, MySQL, Keycloak) containerized separately.
...
\subsection{Flask AI Chatbot Microservice}
\begin{itemize}
    \item Lightweight REST API for AI-powered chatbot interactions.
    \item Integrated with OpenAI GPT for car recommendations and FAQ responses.
    \item Modular architecture for easy maintenance and scaling.
    \item Containerized via Docker, deployed alongside other services.
\end{itemize}
...
\begin{itemize}
    ...
    \item Frameworks: Spring Boot, Flask, Angular, Keycloak.
